...
public interface Embedder {
    /**
     * This is used for the auto-linker to prevent it from linking stuff that'll get processed by an embedder.
     *
    ...
    /**
     * This is used by helper calls in EmbeddingEngine to automatically process a returned result; generally only one of these will be non-null.
     * Note that the htmlDocument can be used to pass arbitrary data in, if necessary, by using new Document("data") and then calling
     * document.getLocation() to retrieve the data.
     *
     * @param jsonReader   A JsonReader that will process calls generated by addCalls, if used
     * @param htmlDocument An HtmlReader that will process calls generated by addCalls, if used
     * @return An embed result for the call, consisting of a title, duration, and an optional extra post image
     */
    EmbedResult parseResult(JsonReader jsonReader, Document htmlDocument)
            throws IOException;
}
